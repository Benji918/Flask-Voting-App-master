...
def candidate_post():
    ...
    error = False
    if not roll_num:
        flash('Roll number must not be empty!', 'error')
        error = True

    elif not 10000000 <= int(float(roll_no)) < 99999999:
    ...
    elif not set(last_name).issubset(string.ascii_letters + " "):
        flash('Name can only contain alphabets.', 'error')
        error = True
    ...
    elif not batch and not course and not department:
    ...
    if error:
        ...
        candidate = CandidateModel(roll_num=roll_num, first_name=first_name, last_name=last_name, batch=batch,
                                   course=course, department=department, post=post, pic_path=pic_path, agenda=agenda)
